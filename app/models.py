...
class Location(db.Model, BaseNestedSets):
    ...
    parent = db.relationship("NotifyingParties", backref="district", lazy='dynamic')
...
class FlowData(db.Model, TimeStampMixin):
    ...
    def __repr__(self):
        return "Flow Data: {}".format(self.id)


class NotifyingParties(db.Model, TimeStampMixin):
    __tablename__ = 'notifying_parties'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(64), index=True)
    msisdn = db.Column(db.String(), index=True)
    district_id = db.Column(db.Integer, db.ForeignKey('locations.id'))
    reporter_type = db.Column(db.String(), index=True)
